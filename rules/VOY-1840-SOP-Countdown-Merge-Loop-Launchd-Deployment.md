...
| 2026-08-09 | Add SECOND local-only full-fidelity audit file `merge-loop.audit.full.jsonl` (§2 files table row, §9 inspection snippet) — raw pr/reason/head/review_decision for every repo, never redacted, never to be shared/pasted; the existing redacted `.audit.jsonl` remains the shareable record and its fail-closed write-ahead contract is unchanged ([#306](https://github.com/iterwheel/voyager/pull/306)) | Claude Code |
