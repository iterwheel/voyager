...
- Issue #214's dedicated PAT fallback is documented in VOY-1827 as a
  hypothesis-level canary SOP. The public registry may record token class,
  actor class, secret backend item name, expiration class, capability booleans,
  operation result, and resolved-by actor class, but must not record token
  material, private PR numbers, or review-thread node IDs.
