...
| `iterwheel-clearance` | `iterwheel/voyager`, `iterwheel/voyager-sandbox` | `pull_request.opened`, `pull_request.edited`, `pull_request.reopened`, `pull_request.ready_for_review`, `pull_request.converted_to_draft`, `pull_request.synchronize`, `pull_request_review.submitted`, `pull_request_review.dismissed`, `pull_request_review_comment.*`, `pull_request_review_thread.resolved`/`.unresolved` (issue #339: manual thread resolution refreshes the stage label), or `/clearance` PR comment | Maintains one PR review-readiness label from `clearance-1-pending`, `clearance-2-blocked`, `clearance-3-ready-for-approval`, and `clearance-4-ready-for-merge`; upserts one Clearance comment; adds `rocket` when ready and `eyes` otherwise |
| `iterwheel-countdown` | `iterwheel/voyager`, `iterwheel/voyager-sandbox` resolver | Manual `vyg countdown resolve-conversation` invocation and Scheduled `vyg countdown resolve-loop` launchd job per VOY-1835 (machine-account, identity-gated). | Resolves PR review conversations as the fixed GitHub user machine account `iterwheel-countdown-bot` (token via `gh auth token --user …`, never printed; this is not a GitHub App `[bot]` actor badge). The only GraphQL mutation issued is `resolveReviewThread`, and only allowlisted repos may be targeted. The earlier dedicated-PAT canary (`review-thread-diagnostic`, VOY-1827/1828/1829) was superseded and removed per VOY-1830. |
...
> resolve-conversation` (machine account `iterwheel-countdown-bot`, identity-gated,
...
| User-to-server refresh route | Verified for non-repository-scoped Device Flow; repo-scoped refresh isolated as GitHub-side/undocumented failure surface | Issue #204 added safe operator helpers for the GitHub App user-to-server route. `vyg countdown user-device-code --client-id <id> --store-refresh-token-command <cmd> [--expected-viewer-login-env <env>] [--repository-id <id>]` starts Device Flow, prints only verification URI, user code, expiry, and polling interval, then polls in-process and sends the first refresh token to the operator-selected secret-store command over stdin. In `--json` mode this long-running command emits JSON Lines: a `device_code` event followed by an `authorization_complete` event. `vyg countdown user-refresh-check --client-id <id> --refresh-token-env <env> --store-refresh-token-command <cmd> [--expected-viewer-login-env <env>]` refreshes from an operator-provided environment variable and sends the replacement refresh token returned by GitHub to the secret-store command over stdin. `--expected-viewer-login-env` compares `viewer.login` with the expected operator login before first-token storage or before replacement-token storage, and public output exposes only `viewer_login_present` and `viewer_login_matches_expected`. On 2026-06-23, after `v0.7.3` was deployed on Wukong, a fresh Device Flow without `--repository-id` completed, stored the first refresh token in the operator Keychain, passed expected-viewer boolean checks, then `user-refresh-check` refreshed successfully and stored the rotated replacement refresh token. The helper still fails closed if the store command fails and does not write plaintext token material. No plaintext token artifacts were written under `~/.voyager`. A prior repository-scoped refresh token continued to return GitHub HTTP 500 with sanitized diagnostics (`content_type`, GitHub request id, and request-shape booleans showing no client secret and no repository id on refresh), so operators should prefer the documented non-repository-scoped refresh path until GitHub documents or resolves repo-scoped refresh behavior. This route must be evaluated as a human-authorized App user credential and must not be described as `iterwheel-countdown[bot]` resolver capability. |
| User-to-server review-thread canary | Complete-negative | On 2026-06-23, `vyg countdown user-review-thread-diagnostic` refreshed the stored GitHub App user credential, stored the replacement refresh token before GraphQL diagnostics, and queried a private sandbox review thread with token-redacted output. Sanitized query evidence: actor login present, target repo/PR matched, type `PullRequestReviewThread`, `isResolved=false`, `isOutdated=false`, `viewerCanReply=true`, and `viewerCanResolve=true`. The guarded `--resolve` attempt failed with sanitized GraphQL `first_type=FORBIDDEN`, `first_message=Resource_not_accessible_by_integration`; after-state remained `isResolved=false`. This is not resolver-capable evidence. Do not use the user-to-server token path as the default review-thread resolver without a new GitHub-supported hypothesis and passing mutation canary. |
| Dedicated PAT fallback | Sandbox resolve canary successful; production gate proposed | Issue #214 explores a dedicated machine-user PAT fallback after App installation tokens and the App user-to-server route failed to produce a usable resolver path. Fine-grained PAT setup must first check the resource owner selector. If the `iterwheel` organization is not selectable, the fine-grained path is not the right canary for an organization-owned private sandbox repository. VOY-1827 records the constrained fallback: a 30-day-or-less classic PAT with only `repo` scope, owned by a dedicated machine user with write access only to the sandbox private repository, stored in macOS Keychain service `voyager/countdown-dedicated-pat`, and used only after Clearance has supplied semantic evidence and the Countdown App token still reports `viewerCanResolve=false`. On 2026-06-25, `vyg countdown review-thread-diagnostic --pat-token-command` queried a private sandbox `PullRequestReviewThread` as the dedicated machine-user actor and returned `viewerCanResolve=true`, `viewerCanReply=true`, `isResolved=false`, and `isOutdated=false`. The Wukong VOY-1828 same-thread canary then confirmed the Countdown App baseline remained not resolve-capable (`viewerCanResolve=false`, `viewerCanReply=true`, unresolved, non-outdated), the dedicated PAT expected-login gate passed, the PAT leg reported `viewerCanResolve=true`, the guarded mutation applied, and after-state was resolved. This proves a dedicated machine-user fallback route, not `iterwheel-countdown[bot]` resolver capability. VOY-1829 defines the default-off production gate. |
| Resolve canary | App route blocked; dedicated PAT fallback successful in sandbox | Countdown installation-token attempts remain blocked by `viewerCanResolve=false`. The user-to-server route can report `viewerCanResolve=true`, but GitHub still rejects `resolveReviewThread` with `Resource not accessible by integration`. The dedicated machine-user PAT fallback route has successful sandbox mutation evidence and remains gated by VOY-1829 before any production expansion. |
...
- Issue #204 must not publish GitHub App user access tokens, refresh tokens,
  ...
  Expected-viewer checks may publish only boolean match metadata. A successful
  user-to-server route proves only that an explicitly authorized maintainer user
  can mint and refresh App user tokens, but the 2026-06-23 user-to-server
  review-thread canary is negative for resolver use because the mutation is
  rejected by GitHub even when `viewerCanResolve=true`.
- The issue #204 live test completed on 2026-06-23 with Voyager `v0.7.3` on
  Wukong. Treat the documented, non-repository-scoped Device Flow path as the
  known-good operator route. Do not use the optional `--repository-id` path for
  refresh evidence unless a later GitHub documentation update or support answer
  explains the observed repo-scoped refresh HTTP 500 behavior.
- Issue #214's dedicated PAT fallback is documented in VOY-1827 as a
  hypothesis-level canary and VOY-1828 as the Wukong operator SOP. The public
  registry may record token class, actor class, secret backend item name,
  expiration class, capability booleans, operation result, and resolved-by
  actor class, but must not record token material, private PR numbers, or
...
| 2026-06-25 | Recorded successful VOY-1828 dedicated PAT fallback sandbox resolve canary and linked VOY-1829 production gate | Codex |
...
| 2026-06-23 | Updated issue #204 status after live Wukong verification: non-repository-scoped Device Flow authorization, Keychain storage, refresh rotation, and replacement-token storage all passed on Voyager `v0.7.3`; repo-scoped refresh remains isolated as a GitHub-side/undocumented failure surface | Codex |
| 2026-06-25 | Linked issue #214 dedicated machine-user PAT fallback to VOY-1827 and recorded the fine-grained PAT resource-owner constraint plus classic PAT canary boundaries | Codex |
| 2026-06-25 | Recorded the dedicated PAT Keychain service name and no-secret-output credential preflight results for issue #214 | Codex |
| 2026-06-25 | Added the dedicated PAT fallback CLI entry point and recorded the private sandbox PAT capability query result; resolve remains pending same-thread App-token baseline evidence | Codex |
| 2026-06-25 | Linked VOY-1828 as the Wukong runbook for the same-thread App baseline and guarded dedicated PAT resolve canary | Codex |
| 2026-06-28 | Renamed the Countdown fixed machine-user resolver login to `iterwheel-countdown-bot` and documented that it is not a GitHub App `[bot]` actor badge | Codex |
| 2026-06-28 | Added VOY-1835 scheduled `vyg countdown resolve-loop` launchd deployment to the Countdown registry row for issue #229 | Codex |
