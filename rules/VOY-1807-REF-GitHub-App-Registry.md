...
| Dedicated PAT fallback | Hypothesis; PAT capability query complete, Wukong resolve canary pending | Issue #214 explores a dedicated machine-user PAT fallback after App installation tokens and the App user-to-server route failed to produce a usable resolver path. Fine-grained PAT setup must first check the resource owner selector. If the `iterwheel` organization is not selectable, the fine-grained path is not the right canary for an organization-owned private sandbox repository. VOY-1827 records the constrained fallback: a 30-day-or-less classic PAT with only `repo` scope, owned by a dedicated machine user with write access only to the sandbox private repository, stored in macOS Keychain service `voyager/countdown-dedicated-pat`, and used only after Clearance has supplied semantic evidence and the Countdown App token still reports `viewerCanResolve=false`. On 2026-06-25, a no-secret-output preflight read the Keychain item, confirmed the OAuth scopes header was `repo`, confirmed sandbox repository permissions included `push=true`, and confirmed `iterwheel/voyager` was public-only for this actor with `push=false`. The same day, `vyg countdown review-thread-diagnostic --pat-token-command` queried a private sandbox `PullRequestReviewThread` as the dedicated machine-user actor and returned `viewerCanResolve=true`, `viewerCanReply=true`, `isResolved=false`, and `isOutdated=false`. No `resolveReviewThread` mutation was run because this workstation did not have Countdown App config or private-key material to collect the same-thread App-token baseline first. VOY-1828 is the Wukong SOP for collecting that same-thread App baseline and, only if both gates match, running the dedicated PAT resolve canary. A successful resolve canary would prove a dedicated machine-user fallback route, not `iterwheel-countdown[bot]` resolver capability. |
| Resolve canary | Blocked by GitHub mutation permission | Countdown installation-token attempts remain blocked by `viewerCanResolve=false`. The user-to-server route can report `viewerCanResolve=true`, but GitHub still rejects `resolveReviewThread` with `Resource not accessible by integration`. No current credential path has resolver-capable evidence. |
