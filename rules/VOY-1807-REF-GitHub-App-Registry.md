...
| User-to-server refresh route | Complete | Issue #204 adds safe operator helpers for the GitHub App user-to-server route. `vyg countdown user-device-code --client-id <id> --store-refresh-token-command <cmd> [--expected-viewer-login-env <env>] [--repository-id <id>]` starts Device Flow, prints only verification URI, user code, expiry, and polling interval, then polls in-process and sends the first refresh token to the operator-selected secret-store command over stdin. In `--json` mode this long-running command emits JSON Lines: a `device_code` event followed by an `authorization_complete` event. `vyg countdown user-refresh-check --client-id <id> --refresh-token-env <env> --store-refresh-token-command <cmd> [--expected-viewer-login-env <env>]` refreshes from an operator-provided environment variable and sends the replacement refresh token returned by GitHub to the secret-store command over stdin. `--expected-viewer-login-env` compares `viewer.login` with the expected operator login before first-token storage or before replacement-token storage, and public output exposes only `viewer_login_present` and `viewer_login_matches_expected`. `--repository-id` requests a repository-restricted Device Flow exchange when GitHub accepts that parameter. The route fails closed on store failures and does not write plaintext token material. On 2026-06-23, the non-repository-scoped Device Flow route completed on Wukong with expected-viewer booleans true, refresh-token storage true, refresh-check replacement storage true, and no plaintext token artifacts. This route must be evaluated as a human-authorized App user credential and must not be described as `iterwheel-countdown[bot]` resolver capability. |
...
- Issue #204 must not publish GitHub App user access tokens, refresh tokens,
  ...
  can mint and refresh App user tokens. The 2026-06-23 user-to-server
