...
1. Land a deterministic fake subprocess backend that proves the non-dry-run Assembly branch, pull-request, Codex-trigger, and progress-comment path without invoking real Pi.
2. After that implementation merges and deploys, run the first real `pi -> oh-my-pi -> DeepSeek V4 Pro` canary only on `iterwheel/voyager-sandbox`.

This CHG does not enable real Pi execution for `iterwheel/voyager`, `frankyxhl/alfred`, or `frankyxhl/trinity`.

---

## Why

Assembly currently routes `/assembly`, validates Blueprint/Stack/actor gates, and records a dry-run job contract. The real backend is still a placeholder: `PiOhMyPiDeepSeekAdapter.execute()` raises `NotImplementedError`.

Before connecting a coding agent that can modify repositories, Voyager needs a deterministic backend that exercises the GitHub mutation path end to end under tests. That gives us confidence in the contract, token boundary, idempotency, failure comments, and PR creation behavior before adding Pi's real subprocess behavior.
...
- A fake subprocess backend exists and is selectable without invoking real Pi.
...
- Real Pi backend remains disabled for production repositories until a sandbox canary passes.
- The sandbox canary plan names `iterwheel/voyager-sandbox` as the first real Pi target and includes rollback to `dry-run`.
...
| D3 | Fake subprocess backend is test-only / local-only unless explicitly selected. | It proves dispatcher behavior without requiring Pi, DeepSeek, network, or repository mutation by a real agent. |
...
| D5 | Real Pi canary is sandbox-only. | The first real coding backend run must not target production repositories. |
...
| D11 | VOY-1807 updates document backend selection and the Pi canary gate only. | The App already exists; this CHG must not imply a new GitHub App is being created. |
...
6. **Fake output safety.** The fake backend reads controlled env/test inputs and returns `AdapterResult` variants without invoking Pi. It must validate fake commit SHAs before returning them; malformed, missing, or non-list SHAs produce `status="failed"` with no commits.
...
10. **Docs/config.** Update `config.example.toml`, VOY-1807 registry notes, and any Assembly SOP text needed to document fake backend and sandbox-only Pi canary. VOY-1807 changes are limited to backend selection and canary gate notes; no new GitHub App is created.
...
14. **Sandbox Pi canary.** Verify `pi` and `oh-my-pi` exist on Wukong, record the exact commands used for verification, obtain Frank Xu approval, set sandbox-only env, trigger one `iterwheel/voyager-sandbox` Assembly issue, and record the result.
...
2. `command -v pi` and `command -v oh-my-pi` succeed on Wukong; record the exact command output in the execution log.
3. Frank Xu approves the sandbox canary before real Pi env is enabled.
4. `BRIDGE_ALLOWED_REPOSITORIES_ITERWHEEL_ASSEMBLY` includes only `iterwheel/voyager-sandbox` for the real Pi canary window.
...
- [ ] Sandbox canary approved by: Frank Xu before real Pi env is enabled
...
- Real Pi canary approval is still pending. The real canary has not run and remains limited to `iterwheel/voyager-sandbox` after merge and deploy.
...
| 2026-05-23 | Confirmed real Pi canary status. | Not run and not approved. It remains sandbox-only for `iterwheel/voyager-sandbox` after merge/deploy; production repositories stay on non-real-Pi execution. |

---

## Post-Change Review

- Did fake subprocess prove the branch/PR/Codex/progress-comment path without real Pi?
- Did token-safety tests catch all rendered/persisted surfaces?
- Did sandbox Pi canary complete without touching production repositories?
- Are follow-up issues needed for broader rollout or Pi ergonomics?
