...
- **Assembly subprocesses no longer inherit the ambient environment (BEHAVIOR CHANGE).** OMP and verification commands now run inside the minimal allow-listed environment (`voyager/bots/assembly/execution_env.py`: locale, `PATH`, temp dirs, `XDG_*`, SSL cert paths, plus git transport vars); OMP additionally receives only `DEEPSEEK_API_KEY`, and verification commands receive the same scrubbed environment. Integrations that depended on ambient variables (e.g. `DATABASE_URL`) silently lose that configuration on upgrade — pass such settings explicitly to the command (flags/config files) instead of relying on inherited env.
