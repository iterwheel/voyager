...
- **Clearance author wake-up (default OFF).** `[clearance.author_wakeup]` (config) / `CLEARANCE_AUTHOR_WAKEUP_*` (Wukong bridge env, see `deploy/wukong/bridge.env.example`). When enabled and allowlisted per repository (`CLEARANCE_AUTHOR_WAKEUP_ALLOWED_REPOSITORIES`), Clearance notifies the PR author's agent door (`CLEARANCE_AUTHOR_WAKEUP_PFC_DOOR_URL`, default `http://localhost:8420/api/agent-send`) when a verdict is held for their PR. Timers run from their actual boundaries: `NOTIFY_AFTER_MINUTES` (10) is measured from Voyager's first durable observation of the held verdict (`first_seen`), and `FALLBACK_AFTER_MINUTES` (20) starts only at a terminal `author_delivered` receipt — a pending or failed delivery postpones or entirely disarms the fallback claim deadline. Delivery follows the durable send-ID retention contract (`REQUIRED_SEND_ID_RETENTION_SECONDS` = 86400, plus receipt poll/timeout, repost-safety-margin, and attempt caps as named in the template). **`CLEARANCE_AUTHOR_WAKEUP_AUTO_REVIEW_FIX` stays `false`** until end-to-end notification delivery is proven. Rollback: `ENABLED=false` — the subsystem is inert and Clearance verdicts behave exactly as before.
- **E2E debug endpoint now gated (BREAKING for tooling).** `GET /e2e/recent_writebacks` (pairs with `scripts/e2e/run_matrix.py`) no longer responds on a public tunnel by default. All of the following are required: `VOYAGER_E2E_DEBUG=1` (otherwise 404 — the endpoint's existence is not leaked), a loopback client IP unless `VOYAGER_E2E_ALLOW_NON_LOOPBACK=1` is set explicitly, and a mandatory `VOYAGER_E2E_TOKEN` matched against the request's `X-Voyager-E2E-Token` header (constant-time compare; responses carry `Cache-Control: no-store`). E2E harness runs must now configure the token; rollback = unset `VOYAGER_E2E_DEBUG` (endpoint fully dark again).
- **Countdown trigger allow-list + scheduler update (required).** `BRIDGE_ALLOWED_REPOSITORIES_ITERWHEEL_COUNTDOWN` (e.g. `iterwheel/voyager`) in the bridge env. Without this entry the server gate default-denies in production (`DRY_RUN=false`) and the Countdown trigger path silently never fires. **Existing Wukong installations must also reinstall the machine-local scheduler wrapper** (`~/.voyager/bin/countdown-resolve-loop-adaptive.sh`) from `deploy/wukong/` per VOY-1835 (always overwrite — it is code, not operator state): trigger detection exists only in the revised template, and `COUNTDOWN_TRIGGER_PATH` must stay identical in both env files, otherwise the bridge can create trigger markers the deployed scheduler never observes. Rollback: set `BRIDGE_ALLOWED_REPOSITORIES_ITERWHEEL_COUNTDOWN` to an explicit empty value, or remove the repository from every fallback source — merely unsetting the per-agent variable falls through to the global `BRIDGE_ALLOWED_REPOSITORIES` env var or the `[bridge.allowed_repositories]` config entry (`repository_gate.py`), which can leave Countdown enabled (possibly with broader scope).
- **`/clearance` now requires an authorized actor (BEHAVIOR CHANGE).** Explicit `/clearance` commands (#253) are routed only after actor authorization — the same gate as Assembly (VOY-1818). Bot accounts are always refused, and `NONE`-association contributors are refused unless on the configured human allow-list. Integrations that previously posted `/clearance` as a bot or unassociated contributor silently stop triggering evaluation on upgrade; migrate the command to a trusted human association or add the operator to the human allow-list. Related resolution semantics also tightened (#253/#254): a PR author's substantive reply alone no longer resolves a Clearance thread (it routes to `NEEDS_HUMAN_JUDGMENT` pending corroboration), and even an investigator's `RESOLVED` verdict is capped to human judgment unless a recorded head-SHA transition postdates the thread's latest activity. Workflows that relied on reply-then-resolve must instead land one of the exact auto-resolve signals — the 👍 reaction, or the formal `Codex Review:` clean-verdict comment in its full prefix-anchored form; ordinary approval prose (e.g. “Looks good”) no longer qualifies (#334 fail-safe asymmetry) — or resolve the thread manually, or land the corroborating head transition.
- **Assembly subprocesses no longer inherit the ambient environment (BEHAVIOR CHANGE).** OMP and verification commands now run inside the minimal allow-listed environment (`voyager/bots/assembly/execution_env.py`: locale, `PATH`, temp dirs, `XDG_*`, SSL cert paths); OMP additionally receives only `DEEPSEEK_API_KEY`. Proxy and Git transport variables (`HTTPS_PROXY`/`HTTP_PROXY`/`NO_PROXY`, `GIT_SSL_*`, …) are passed ONLY to Git subprocesses (`scoped_git_env()`) — the OMP endpoint and verification commands do not see them. Integrations that depended on ambient variables (e.g. `DATABASE_URL`, or a proxy for outbound HTTP) silently lose that configuration on upgrade — pass such settings explicitly to the command (flags/config files) instead of relying on inherited env.
- **Known limitation: OpenAI SDK capped below 3.** `openai>=2.36,<3` is pinned pending the deliberate HTTPX2 migration tracked in #321. Deployments or integrations that require OpenAI 3 will fail dependency resolution or be forced to downgrade on upgrade to 0.12.0; the cap lifts when #321 lands. Dependabot is configured to ignore `version-update:semver-major` bumps for `openai` in the interim.
- **Known behavior: Assembly commits are unsigned and hook-free.** Generated Assembly commits are created with `git commit --no-gpg-sign --no-verify`, so targets that require signed commits (branch-protection signed-commit rules) will reject the push, and commit-hook-produced changes (formatting, generated files) are not applied. This is deliberate — model-written code must not execute local hooks or sign with ambient keys; targets needing either should run their checks in CI/verification commands instead of local hooks.
