...
def test_runner_escalates_when_fix_result_tests_is_bare_string(tmp_path) -> None:
    audit_path = tmp_path / "review-fix.jsonl"
    ...
    assert records[-1].tests == (
        "page-human-reviewer",
        "fix_error=ReviewFixLoopRunnerError: tests must be a non-string sequence of strings",
    )
    assert tuple("pytest") not in {record.tests for record in records}
